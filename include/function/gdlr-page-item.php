	...
	*/
	...
	if( !function_exists('gdlr_get_item_title') ){
		function gdlr_get_item_title( $atts ){
			...
			if( !empty($atts['title-type']) && $atts['title-type'] != 'none' ){
				$item_class  = 'pos-' . str_replace('-large', '', $atts['title-type']);
				$item_class .= (strpos($atts['title-type'], 'large') > 0)? ' gdlr-large': '';
	...
	if( !function_exists('gdlr_get_service_with_image_item') ){
		function gdlr_get_service_with_image_item( $settings ){
			...
			$ret .= '<div class="service-with-image-divider gdlr-title-background"></div>';
	...
	if( !function_exists('gdlr_get_divider_item') ){
		function gdlr_get_divider_item( $settings ){
			...
			$color = !empty($settings['color'])? 'border-color: ' . $settings['color'] . '; ': '';
			$size = !empty($settings['size'])? 'width: ' . $settings['size'] . '; ': '';
			$style = (!empty($size) || !empty($color))? ' style="' . $size . $color . '" ': '';
	...
	if( !function_exists('gdlr_get_twitter_item') ){
		function gdlr_get_twitter_item( $settings ){
			...
			$ret .= '<div class="gdlr-twitter-item-head gdlr-nav-container" >';
			$ret .= '<i class="icon-angle-left gdlr-flex-prev"></i>';
			$ret .= '<i class="icon-twitter"></i>';
			$ret .= '<i class="icon-angle-right gdlr-flex-next"></i>';
			$ret .= '</div>';
			
			$ret .= '<div class="gdlr-twitter-item-wrapper" >';
			$ret .= '<div class="flexslider" data-type="carousel" data-nav-container="gdlr-twitter-item-wrapper" >';
			$ret .= '<ul class="slides" >';
			...
			$ret .= '</ul>';			
			$ret .= '</div>';
			$ret .= '</div>'; // gdlr-twiiter-item-wrapper
			$ret .= '</div>'; // gdlr-twiiter-item 
	...
	// author item
	if( !function_exists('gdlr_get_author_item') ){
		function gdlr_get_author_item( $settings ){	
			$item_id = empty($settings['page-item-id'])? '': ' id="' . $settings['page-item-id'] . '" ';
			...
			$settings['order'] = !empty($settings['order'])? $settings['order']: 'name';
			$settings['orderby'] = !empty($settings['orderby'])? $settings['orderby']: 'asc';
			$user_query = new WP_User_Query(array('number'=>'10', 'offset'=>'0', 
				'orderby'=>$settings['orderby'], 'order'=>$settings['order']));
			
			$item_size = intval(str_replace('1/', '', $settings['column'])); $current_size = 0;
			$ret  = '<div class="gdlr-author-item-wrapper" ' . $item_id . $margin_style . '>';
			if( !empty($user_query->results) ){
				foreach( $user_query->results as $user ){ 
					...
					$ret .= '<div class="' . gdlr_get_column_class($settings['column']) . '">';
					$ret .= '<div class="gdlr-item gdlr-author-item">';
					
					$ret .= '<img src="' . get_user_meta($user->ID, 'authorimage', true) . '" alt="" />';
					$ret .= '<div class="gdlr-author-content-wrapper">';
					$ret .= '<h4 class="gdlr-author-title">' . $user->display_name . '</h4>';
					$ret .= '<div class="gdlr-author-content">' . get_user_meta($user->ID, 'description', true) . '</div>';
					$ret .= '<a class="gdlr-button with-border" href="' . get_author_posts_url($user->ID) . '">' . __('Read Articles', 'gdlr_translate') . '</a>';
					$ret .= gdlr_get_author_social($user->ID);
					$ret .= '</div>'; // gdlr-author-content-wrapper
