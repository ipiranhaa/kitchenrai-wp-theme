<?php
	...
	*/	
			...
			while($query->have_posts()){ $query->the_post();
				...
				if( !empty($image_id) ){
					...
					if( $settings['style'] == 'no-excerpt' ){
						$slide_data[$image_id]['caption']  = '<div class="gdlr-caption-date gdlr-slider-title-font" >';
						...
						$slide_data[$image_id]['caption'] .= '<span class="gdlr-button with-border">' . __('Read More', 'gdlr_translate') . '</span>';	
			...
			foreach($slides as $slide_id => $slide){
				...
				if( is_array($slide) ){
					...
					if( !empty($slide['title']) || !empty($slide['caption']) ){
						...
						$caption .= '<div class="gdlr-caption-inner" >';
						$caption .= '<div class="gdlr-caption">';
						$caption .= empty($slide['title'])? '': '<h4 class="gdlr-caption-title gdlr-slider-title-font">' . $slide['title'] . '</h4>';
						$caption .= empty($slide['caption'])? '': '<div class="gdlr-caption-text">' . $slide['caption'] . '</div>';
						$caption .= '</div>'; // gdlr-slider-caption
						$caption .= '</div>'; // gdlr-slider-caption-wrapper
						$caption .= '</div>';
	...
	if( !function_exists('gdlr_get_gallery_grid') ){
		function gdlr_get_gallery_grid( $settings ){
			$item_id = empty($settings['page-item-id'])? '': ' id="' . $settings['page-item-id'] . '" ';

			global $gdlr_spaces, $gdlr_gallery_id; $gdlr_gallery_id++;
			$margin = (!empty($settings['margin-bottom']) && 
				$settings['margin-bottom'] != $gdlr_spaces['bottom-item'])? 'margin-bottom: ' . $settings['margin-bottom'] . ';': '';
			$margin_style = (!empty($margin))? ' style="' . $margin . '" ': '';			
			
			$ret  = '<div class="gdlr-gallery-grid gdlr-item " ' . $item_id . $margin_style . '>';
			
			// full image
			$count = 0;
			$ret .= '<div class="gdlr-gallery-grid-container">';
			foreach($settings['slider'] as $slide_id => $slide){ $count++;
				if( $count % 6 == 1 || $count % 6 == 4 ){
					$ret .= '<div class="gallery-grid-first">';
					$ret .= '<div class="gallery-grid-first-inner">';
					$ret .= gdlr_get_image($slide_id, 'blog-gallery-first', array('id'=>$gdlr_gallery_id));
					$ret .= '</div>';
					$ret .= '</div>';
				}else if( $count % 6 == 2 ){
					$ret .= '<div class="gallery-grid-second">';
					$ret .= '<div class="gallery-grid-second-inner">';
					$ret .= gdlr_get_image($slide_id, 'blog-gallery-second', array('id'=>$gdlr_gallery_id));
					$ret .= '</div>';
				}else if( $count % 6 == 3 ){
					$ret .= '<div class="gallery-grid-second-inner">';
					$ret .= gdlr_get_image($slide_id, 'blog-gallery-second', array('id'=>$gdlr_gallery_id));
					$ret .= '</div>';
					$ret .= '</div>';
				}else if( $count % 6 == 5 || $count % 6 == 0 ){
					$ret .= '<div class="gallery-grid-five">';
					$ret .= '<div class="gallery-grid-five-inner">';
					$ret .= gdlr_get_image($slide_id, 'blog-gallery-third', array('id'=>$gdlr_gallery_id));
					$ret .= '</div>';					
					$ret .= '</div>';					
				}
				
				if( $count % 6 == 0 || $count % 6 == 4 ){ $ret .= '<div class="clear"></div>'; }
			}
			if( $count % 6 == 2 ){ $ret .= '</div>'; }
			$ret .= '</div>';
